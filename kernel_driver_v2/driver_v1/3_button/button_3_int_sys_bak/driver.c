...
static int driver_test_init(void)
{
	...
	ret = request_irq(irq_num1, button_irq_handler, IRQF_TRIGGER_FALLING, "tiny4412_key1", (void *)"key1");
	if (ret) {
		printk("Init: request_irq irq_num1 failed\n");
	}
	
	ret = request_irq(irq_num2, button_irq_handler, IRQF_TRIGGER_FALLING, "tiny4412_key2", (void *)"key2");
	if (ret) {
		printk("Init: request_irq irq_num2 failed\n");
	}
	
	ret = request_irq(irq_num3, button_irq_handler, IRQF_TRIGGER_FALLING, "tiny4412_key3", (void *)"key3");
	if (ret) {
		printk("Init: request_irq irq_num3 failed\n");
	}
	
	ret = request_irq(irq_num4, button_irq_handler, IRQF_TRIGGER_FALLING, "tiny4412_key4", (void *)"key4");
	if (ret) {
