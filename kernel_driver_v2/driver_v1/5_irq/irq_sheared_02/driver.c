...
static irqreturn_t button0_irq_handler(int irq, void *dev_id)
{
	printk("Driver: %s, irq: %d\n", __func__, irq);
...
int driver_test_open (struct inode *inodep, struct file *filp)
{
	...
	for (i = 0; i < ARRAY_SIZE(key_irq); i++)
	{
		...
		ret = request_irq(key_irq[i].irq, button0_irq_handler, IRQF_TRIGGER_FALLING | IRQF_SHARED, key_irq[i].name, (void *)&key_irq[i]);
		if (ret) {
